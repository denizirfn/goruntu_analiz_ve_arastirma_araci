...
class WebScraper:
    """
    Web araması yapıp, sonuçlardaki sayfaların metin içeriğini çıkaran sınıf.
    """
    def search_and_extract(self, query: str, num_results: int = MAX_SEARCH_RESULTS) -> Optional[str]:
        """
        Verilen sorgu ile DuckDuckGo'da arama yapar ve ilk birkaç sonucun metin içeriğini birleştirir.
        """
    ...
    def _fetch_and_parse_url(self, url: str) -> Optional[str]:
        """Belirli bir URL'den HTML içeriğini alır ve metni çıkarır."""
        ...
        try:
            ...
            # İçerik çok büyükse işlemi durdur (isteğe bağlı güvenlik önlemi)
            if len(response.content) > 5_000_000: # Örn: 5 MB limit
            ...
            # Gereksiz elementleri kaldır (script, style vb.)
            ...
            # Metinleri çıkar, satırları birleştir, fazla boşlukları temizle
            page_text = main_content.get_text(separator='\n', strip=True)
            page_text = re.sub(r'\n\s*\n', '\n', page_text).strip()

            if page_text and len(page_text) > 50: # Çok kısa içerikleri de atlayabiliriz (örn. sadece menü kalmışsa)
                print(f"     [WebScraper] Metin çıkarıldı (yaklaşık {len(page_text)} karakter).")
                # Metni belirlenen maksimum uzunlukla sınırla
